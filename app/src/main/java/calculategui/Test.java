...
public class Test {
    ...
    public static void main(String[] args) {
        ...
        findCombination(solution, 0, BigDecimal.ZERO, 0);
        if (minUniqueCount < Integer.MAX_VALUE) {
    ...
    private static boolean findCombination(int[] solution, int index, BigDecimal currentSum, int uniqueCount) {
        ...
        for (int i = 0; i <= maxUse; i++) {
            solution[index] = i;
            if (findCombination(solution, index + 1, currentSum.add(numbers[index].multiply(BigDecimal.valueOf(i))), uniqueCount + (i > 0 ? 1 : 0))) {
                return true;
            }
        }
